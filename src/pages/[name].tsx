...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  // fallback: false => 빌드 타임에 모든 경로를 만들고 만약 찾을 수 없는 경로에 도달한다면 바로 404 페이지를 띄우게 됨

  // fallback: true => 빌드 타임에 모든 경로를 만들고 존재하지 않는 경로에 접근하더라도 바로 404 페이지가 보이지 않음
  ...
  // fallback: "blocking" => getStaticProps 함수가 return 될때까지 UI를 가만히 blocking 함
